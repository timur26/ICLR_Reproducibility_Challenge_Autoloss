...
class DCGAN(object):
  def __init__(self, sess, input_height=108, input_width=108, crop=True,
    ...
    if config.dataset == 'mnist':
      self.eval_size = 5000
      self.reward_every = 20

    else:
      self.eval_size = 500
      self.reward_every = 200
    ...
    self.C = config.C 
    self.k = config.k
    self.learning_rate_c = config.learning_rate_c
  ...
  def build_model(self):
    ...
    print("PRINTING PARAMS of Controller")
    print(self.batch_size)
    print(self.T)
    print(self.C)
    print(self.k)
    print(self.learning_rate_c)
    ##Initialize Controller
    self.controller = autoLoss(sess=self.sess, size_D=self.size_D, size_G=self.size_G, batch_size=self.batch_size, T=self.T, C=self.C, k=self.k, LR=self.learning_rate_c)

    self.saver = tf.train.Saver()

  def train(self, config): 
    # Create D optimizer.
    d_optim = tf.train.AdamOptimizer(config.learning_rate*config.D_LR, beta1=config.beta1)
    # Compute the gradients for a list of variables.
    grads_d_and_vars = d_optim.compute_gradients(self.d_loss, var_list=self.d_vars)
    # Ask the optimizer to apply the capped gradients.
    update_d = d_optim.apply_gradients(grads_d_and_vars)  

    # Create G optimizer.
    g_optim = tf.train.AdamOptimizer(config.learning_rate*config.G_LR, beta1=config.beta1)
    # Compute the gradients for a list of variables.
    grads_g_and_vars = g_optim.compute_gradients(self.g_loss, var_list=self.g_vars)
    # Ask the optimizer to apply the capped gradients.
    ...
    ## If you want to start saving checkpoint. Consumes memory and no fresh starts!
    ...
    for epoch in xrange(config.epoch):
      ...
      for idx in xrange(0, int(batch_idxs)):
        ...
        ######Plus, Feature 2 #######
        if self.y_D:
          _, new_grads_d_and_vars, summary_str, D_real, D_fake = self.sess.run([update_d, grads_d_and_vars, self.d_sum, self.D, self.D_],
        ...
        ######Plus, Feature 2 #######
        if self.y_G:
          _, new_grads_g_and_vars, summary_str = self.sess.run([update_g, grads_g_and_vars, self.g_sum],
...
#####################################################################
        # autoLoss Extract Features to train controller
#####################################################################  


        # ## Feature Number 3 -- Losses of D and G
        ...
        if t % self.reward_every == 0:
          ...
          if config.dataset == 'cifar10':
            ...
            self.perf_G = inception_real_CIFAR  

        perf_D_fake = self.performance_D(D_fake, batch_type=0)
        perf_D_real = self.performance_D(D_real, batch_type=1)
        new_perf_D = 0.5 * (perf_D_real + perf_D_fake)
        self.perf_D = new_perf_D

########################################################################
        # # autoLoss.schedule()
        # ##Get which Component to update next
        # ## If y_D == 1; Update D. Else G
        self.y_D = self.controller.step_controller(t, self.grad_D, self.grad_G, self.errD_real, self.errD_fake, self.errG, self.perf_G, self.perf_D)
        self.y_G = 1 - self.y_D
########################################################################

######################################################################
        # # #Update the controller every T steps
        reward_term = self.perf_G
        if t % self.controller.T == 0:
          self.controller.update_controller(reward_term, append)
######################################################################

        counter += 1
        print("Epoch: [%2d/%2d] [%4d/%4d] time: %4.4f, d_loss: %.4f, g_loss: %.4f, perf_D: %.4f, perf_G: %.4f" \
          % (epoch, config.epoch, idx, batch_idxs,
            time.time() - start_time, self.errD_fake+self.errD_real, self.errG, self.perf_D, self.perf_G))

        ## At every 100 batch iterations. Save Image
        ...
        ## At every 500 epochs, save checkpoints
        #TO SAVE MODEL
...
###########################################################
  ...
  def save_scores(self, incp_score, fid_score, config):
    ...
    if config.dataset == 'mnist':
      with open('scores/{}/autoLoss_epoch_{}_LRD_{}_LRG_{}_Dit_{}_Git_{}_fid_score.pickle'.format(
                 config.dataset, config.epoch, config.D_LR, config.G_LR, config.D_it, config.G_it), 'wb') as handle:
        pickle.dump(fid_score, handle)

##PAst Useful Debugs ##IGNORE
## Feature Number 4 -- Performance of D and G
## perf_G - IS score
## perf_D - D score
# if t % 15000 == 0:
#   G_eval_images = self.eval_images.eval({self.z: sample_eval_z})
#   new_perf_G = inc_score.get_inception_score(self.sess, G_eval_images)
#   self.perf_G = new_perf_G
# if t % 50 == 0:
# G_eval_score = self.eval_score.eval({self.z: sample_eval_z})
# self.perf_G = G_eval_score
# # self.perf_G = 0