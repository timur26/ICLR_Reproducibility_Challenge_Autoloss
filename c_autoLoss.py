...
class c_autoLoss(object):
    ...
    def controller(self, feature, reuse=False):
        with tf.variable_scope("controller") as scope:
            ...
            y2 = linear(tf.reshape(feature, [1, -1]), 2, 'controller_lin1')
            y1 = tf.nn.relu(y2)
    ...
    def update_controller(self, valid_error):
        ...
        ##HERE VALID_ERROR IS ACTUALLY VALID_PERF
        self.C = 0.9*self.C ##Annealing
        print("C: ", self.C)
        self.reward_term = 1.0 / (1 - valid_error)
        ...
        self.sess.run(self.apply_gradients, feed_dict={self.rew: self.C*(self.reward_term - self.reward_moving_average)})
    ...
    def add_feature(self, t, grad_l1, grad_l2, l1, l2, perf):
        ...
        perf2 = self.perf_moving_average
                                   ...
                                   #1          #2            #2        #3  #3   #4      #4
        new_feature = np.array([[progress, 100*norm_grad_l1, 100*norm_grad_l2, l1, l2, perf1, perf2]])
        # print(new_feature)
        return new_feature
